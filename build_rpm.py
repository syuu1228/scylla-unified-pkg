...
if __name__ == "__main__":
    ...
    subprocess.run('../tools/toolchain/dbuild ./reloc/build_rpm.sh', shell=True, check=True, cwd='scylla')
    subprocess.run('../tools/toolchain/dbuild ./reloc/python3/build_reloc.sh', shell=True, check=True, cwd='scylla')
    subprocess.run('../tools/toolchain/dbuild ./reloc/python3/build_rpm.sh', shell=True, check=True, cwd='scylla')
    subprocess.run('../tools/toolchain/dbuild ./reloc/build_reloc.sh', shell=True, check=True, cwd='scylla-jmx')
    subprocess.run('../tools/toolchain/dbuild ./reloc/build_rpm.sh', shell=True, check=True, cwd='scylla-jmx')
    subprocess.run('../tools/toolchain/dbuild ./reloc/build_reloc.sh', shell=True, check=True, cwd='scylla-tools-java')
    subprocess.run('../tools/toolchain/dbuild ./reloc/build_rpm.sh', shell=True, check=True, cwd='scylla-tools-java')
    for m in ['scylla', 'scylla-jmx', 'scylla-tools-java']:
        ...
        for d in pathlib.Path(m).glob('**/build/redhat/RPMS/*/'):
            os.makedirs('build/redhat/RPMS/{}'.format(d.name), exist_ok=True)
            for f in d.glob('**/scylla*-{}-{}.*.rpm'.format(version, release)):
                shutil.copy(f, 'build/redhat/RPMS/{}/{}'.format(d.name, f.name))
